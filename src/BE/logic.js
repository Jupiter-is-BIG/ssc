...
async function query(session, campus, course) {
    ...
    let searchBar = await DRIVER.findElement(By.id("wd-AdvancedFacetedSearch-SearchTextBox-input"));
...
async function goToSearch(session, campus) {
    ...
    await Util.pressButton(DRIVER, By.css("[data-automation-id='wd-CommandButton_uic_okButton']"));
    await Util.waitToLoad(DRIVER, By.id("wd-AdvancedFacetedSearch-SearchTextBox-input"));
    LAST_CAMPUS_REQUEST = campus;
    LAST_SESSION_REQUEST = session;
}

async function handleSessionEntry(session, campus) {
    await Util.pressButton(DRIVER, By.id("ExternalField146_44940PromptQualifier1"));
    await Util.waitToLoad(DRIVER, By.css("[data-automation-id='multiselectInputContainer'] input"));
...
async function handleLevelEntry() {
    await Util.pressButton(DRIVER, By.id("ExternalField146_44944PromptQualifier1"));
    await Util.waitToLoad(DRIVER, By.css("[data-uxi-element-id='ExternalField146_44944PromptQualifier1'] input"));
